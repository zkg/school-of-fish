...
void setupAudio()
{
  ...
  moog    = new MoogFilter( 1200, 0.1 );
  myDelay = new Delay( 0.2, 0.3, true, true );
  flange = new Flanger( 1,     // delay length in milliseconds ( clamped to [0,100] )
                        0.01f,   // lfo rate in Hz ( clamped at low end to 0.001 )
                        1,     // delay depth in milliseconds ( minimum of 0 )
                        0.1f,   // amount of feedback ( clamped to [0,1] )
  ...
  Noise noize = new Noise( 0.1f );  

  // send the noise through the filter
  noize.patch( moog ).patch( flange ).patch( myDelay ).patch( out );
}

// we'll control the frequency and resonance of the filter
// using the position of the mouse, in typical x-y controller fashion
void mouseMovedAudio()
{
  float freq = constrain( map( mouseX, 0, width, 0, 2000 ), 0, 2000 );
  float rez  = constrain( map( mouseY, height, 0, 0, .5 ), 0, .5 );

  moog.frequency.setLastValue( freq );
  moog.resonance.setLastValue( rez  );
}

void keyPressedAudio()
{
